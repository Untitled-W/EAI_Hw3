...
# Remove all entries starting with '/usr/local/cuda' from PATH and LD_LIBRARY_PATH
os.environ['PATH'] = ':'.join(
    path for path in os.environ.get('PATH', '').split(':') if not path.startswith('/usr/local/cuda')
)
os.environ['LD_LIBRARY_PATH'] = ':'.join(
    path for path in os.environ.get('LD_LIBRARY_PATH', '').split(':') if not path.startswith('/usr/local/cuda')
)
...
def save_video(env, make_inference_fn, params):
    ...
    plt.savefig(f"{save_dir}/part3_LinVel_error.png")
...
def train_ppo():
    ...
    global save_dir
    save_dir = os.path.join("./exps3", timestamp)
