...
class MyGetupEnv(Getup):
    def step(self, state: mjx_env.State, action: jax.Array):
        ...
        reward =  rew_term_1 + rew_term_2 + rew_term_3
    ...
    def reset(self, rng: jax.Array) -> mjx_env.State:
        ...
        metrics = {"reward": jp.zeros(()), "height_error": jp.zeros(()),
                   "orientation_error": jp.zeros(()), "joint_position_error": jp.zeros(()),
                   "angular_velocity_error": jp.zeros(())}
...
def save_experiment(params, metrics):
    import shutil

    # Save checkpoints (parameters and metrics)
    ...
    shutil.copy(current_file_path, os.path.join(save_dir, os.path.basename(current_file_path)))
    print(f"Experiment saved to {save_dir}")


def save_video(env, make_inference_fn, params, tt):
    ...
    plt.savefig(os.path.join(save_dir, f"part2_height_error_{tt}.png"))
    ...
    media.write_video(f'{save_dir}/_video_{tt}.mp4', frames)
...
def train_ppo():
    ...
    # Create a folder under ./exps named with the current time
    timestamp = datetime.now().strftime("%m%d_%H%M")
    global save_dir
    save_dir = os.path.join("./exps2", timestamp)
    os.makedirs(save_dir, exist_ok=True)
    print(f"Making directory {save_dir}")

    ppo_params = create_param()
